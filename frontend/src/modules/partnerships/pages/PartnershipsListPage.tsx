...
import * as partnershipsService from '../services/partnerships.service';
import { Partnerships } from '../types';
import { handleApiError } from '../../../core/utils/api';

export const PartnershipsListPage: React.FC = () => {
  const [data, setData] = useState<Partnerships[]>([]);
  ...
  const loadData = async () => {
    setIsLoading(true);
    try {
      const result = await partnershipsService.getAll();
      setData(result.data);
    } catch (error) {
...
export default PartnershipsListPage;
